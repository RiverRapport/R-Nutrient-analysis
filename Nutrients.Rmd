...
res <- resid(TPM1_ARMA$lme, type = "normalized")  #need to do on normalized resid
acf(res, lag.max = 36, main = "ACF - AR(1) errors")
pacf(res, lag.max = 36, main = "pACF- AR(1) errors")
layout(1)

anova(TPM1_ARMA0$lme,TPM1_ARMA$lme)
```
...
#concurvity check:
concurvity(TPGI,full=TRUE)#if any of the values at worst are higher than 0.8, you want to look at full=FALSE as well.
concurvity(TPGI,full=FALSE)#Look for the worst-case scenario and see if variables with high values have problematic shapes or confidence intervals.
```
...
sm_year_TI <-
  smooth_estimates(TPGI, smooth = "s(year):River_sectionTI")
...
sm <-
  smooth_estimates(TPGI, smooth = "s(Longitude,Latitude)", dist = 0.05)
...
TNM1 <-
  gam(
    TN ~ s(year, k = 10, m = 2, bs = "tp") + s(year, by = River_section, m = 1)+
      s(River_section, bs = 're') +
      s(Data_source, bs = 're') +
      s(DOY, bs = "tp") +
      s(Longitude,Latitude,bs = "ds",m = c(1, .5),k = 10) +
...
TN_GS <- gam(
  TN ~ s(year, m = 2) + s(year, River_section, bs = "fs", m = 2) +
    s(Data_source, bs = 're') +
    s(DOY, bs = "tp") +
    s(Longitude,Latitude, bs = "ds",m = c(1, .5), k = 10) +
    s(NS_MC, bs = 're') +
...
TN_S <- gam(
  TN ~ s(year, River_section, bs = "fs", m = 2) +
    s(Data_source, bs = 're') +
    s(DOY, bs = "tp") +
    s(Longitude,Latitude, bs = "ds",m = c(1, .5), k = 10) +
    s(NS_MC, bs = 're') +
...
concurvity(TNM2, full = FALSE)#Look for the worst-case scenario and see if variables with high values have problematic shapes or confidence intervals.
```
...
#Year partial effects with long + lat
draw(TNM2,select=9,residuals=TRUE,continuous_fill = ggplot2::scale_fill_distiller(palette = "PuOr", type = "div")) &
  theme_bw()
...
tn_gam_plot
ggsave("TP_year_plot.svg",tn_gam_plot,width=12,height=8,dpi=600)
...
icefree_dreis<-Ac_gov %>%                  # Specify original dataframe
...
p2 <-
  ggplot(data = brock, aes(invade, lchltp, fill = invade)) + geom_boxplot() +
  scale_fill_viridis(discrete = TRUE,
                     labels = c("Pre-invasion", "Post-invasion")) +
  labs(fill = "Invasion Status") +
  scale_x_discrete(labels = c('Pre', 'Post')) +
