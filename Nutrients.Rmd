---
title: "Nutrient Indicator - Great River Rapport"
author: "Erin Smith"
date: "2023-02-28"
output:
...
```{r Load Packages, warning=FALSE, error=FALSE,tidy=TRUE,message=FALSE}
library(Kendall)
library(corrplot)
library(trend)
library(segmented)
...
```{r Load and prepare data, warning=F,message=F}
...
Ac_gov$River_section <-
  factor(Ac_gov$River_section , levels = c("TI", "BR", "LSL", "CA", "LSF"))
...
GLIP_USGS <-
  subset(Nut_precip,
         Data_source == "OMECP_GLIP" | Data_source == "USGS-NY")
...
```
## Nutrient species Correlation Matrix
```{r Correlation matrix, message=F}
res <-
  cor(Nut_precip[, c(15, 16, 18, 23, 24, 20)], use = 'pairwise.complete.obs')
corrplot(
  res,
  type = "upper",
  order = "hclust",
...
### Exploratory analysis  By Depth

```{r Kruskal Wallis for depth,warning=F}
...
```
Kruskal Wallis by River Section

```{r Kruskal Wallis for river section, warning=F}
...
```{r Seasonal Mann Kendall, warning=F, message=F}
Seas_Nut<-Nut_precip %>%
  filter(year<2021)  %>%
...
P_SMK<-kendallSeasonalTrendTest(Seas_Nut[[parameter]]~season+year,
                            data=Seas_Nut,na.action=na.pass,
...
for (parameter in c("TP", "SRP")) {
  ...
  x <- icefree$year
  break.value <-
    segmented(lm(y ~ x), seg.Z = ~ x)$psi[, 2]
  break.value
...
```{r N:P temporal plot, message=F, warning=F}
...
N_SMK<-kendallSeasonalTrendTest(Seas_Nut[[parameter]]~season+year,
                            data=Seas_Nut,na.action=na.pass,
                            alternative="two.sided",
                            independent.obs=TRUE)

  SMKresultsN=rbind(SMKresultsN,data.frame(parameter,
                                           data.frame(as.list(N_SMK$estimate), 
...
for (parameter in c("TN", "NO2.3", "NH3.4")) {
  ...
  x <- icefree$year
  break.value <-
    segmented(lm(y ~ x), seg.Z = ~ x, psi = 2000)$psi[, 2]
  break.value
  seg.res <- segmented(lm(y ~ x), seg.Z = ~ x, psi = 2000)
  p1 <-
    pscore.test(
...
```{r Seasonal N plots, warning=F,message=F}
par(mfrow=c(1,3))
  for (parameter in c("TN","NO2.3","NH3.4")) {
print(ggplot(icefree, aes(year,icefree[[parameter]])) +
  geom_point() +
...
```{r Data exploration for GAMs, warning=F, message=F}
...
```{r Subset and autocorrelation checks,warning=F,message=F}
...
TPM1_ARMA<-gamm(TP ~ s(year,k=10, bs="tp")+ s(year,River_section,bs='fs')+ s(River_section,bs="re")+
            ...
            ti(Longitude,Latitude,year,d=c(2,1),bs=c('ds','tp'),m=c(1,0.5))+
            s(NS_MC,bs="re"),
          data=spring,  correlation = corARMA(form = ~1|year,p=1),
          family='tw')
...
```{r TP GAM building, warning =F,message=F}
TPBM<-gam(TP ~ s(year,m=2) + s(year,by=River_section,m=1)+ s(River_section,bs='re')+
            s(DOY,bs="tp")+
            s(NS_MC,bs='re')+
...
TPM2<-gam(TP ~ s(year,m=2) + s(year,by=River_section,m=1)+ s(River_section,bs='re')+
            s(DOY,bs="tp")+
            s(NS_MC,bs='re')+
            s(Longitude, Latitude,bs="ds",m=c(1,.5),k=20)+
            s(Data_source,bs='re')+
...
TPM3<-gam(TP ~ s(year,m=2) + s(year,by=River_section,m=1)+ s(River_section,bs='re')+
            s(DOY,bs="tp")+
            s(NS_MC,bs='re')+
            s(Longitude, Latitude,bs="ds",m=c(1,.5),k=20)+
            s(Data_source,bs='re')+
...
TPM4<-gam(TP ~ s(year,m=2) + s(year,by=River_section,m=1)+ s(River_section,bs='re')+
            s(DOY,bs="tp")+
            s(NS_MC,bs='re')+
            s(Longitude, Latitude,bs="ds",m=c(1,.5),k=20)+
            s(Data_source,bs='re')+
...
draw(TPM3,  select = 10,  residuals = FALSE,
  continuous_fill = ggplot2::scale_fill_distiller(palette = "PuOr", type = "div")
)
draw(TPM3,  select = 11,  residuals = TRUE,
  continuous_fill = ggplot2::scale_fill_distiller(
    palette = "PuOr",
    type = "div",
...
TP_year_plot <- ggplot(sm_year, aes(x = year, y = est)) +
  geom_ribbon(alpha = 0.5,
              aes(ymin = lower, ymax = upper, fill = "confidence interval")) +
  geom_line(data = sm_year,
            aes(x = year, y = est, color = "GAM"),
            linewidth = 1) +
  scale_fill_manual(values = "lightblue", name = NULL) +
  scale_color_manual(values = "blue", name = NULL) +
...
sm_year_TI <-
  smooth_estimates(TPM3, smooth = "s(year):River_sectionTI")
...
p_year_TI <- ggplot(sm_year_TI, aes(x = year, y = est)) +
  ...
  geom_ribbon(aes(ymin = lower, ymax = upper, fill = "confidence interval"),
              alpha = 0.5) +
  geom_line(data = sm_year_TI,
            aes(x = year, y = est, color = "GAM"),
            linewidth = 1) +
  scale_fill_manual(values = "lightblue", name = NULL) +
...
sm <-
  smooth_estimates(TPM3, smooth = "s(Longitude,Latitude)", dist = 0.05)
...
TNM1 <-
  gam(
    TN ~ s(year, k = 20, m = 2, bs = "tp") + s(year, by = River_section, m =1) +
      s(River_section, bs = 're') +
      s(Data_source, bs = 're') +
      s(DOY, bs = "tp") +
      s(Longitude,Latitude,bs = "ds", m = c(1, .5), k = 10) +
      s(NS_MC, bs = 're'),
...
TNM1_ARMA0 <-
  gamm(
    TN ~ s(year, k = 10, bs = "tp") + s(year, River_section, bs = 'fs') +
      s(River_section, bs="re") +
      s(Data_source, bs = 're') +
      s(DOY, bs = "tp") +
      s(Longitude,Latitude,bs = "ds",m = c(1, .5),k = 10) +
...
auto.arima(
  resid(TNM1_ARMA0$lme, type = "normalized"),
  stationary = TRUE,
  seasonal = FALSE
)
...
TNM1_AR1 <-
  gamm(
    TN ~ s(year, k = 10, bs = "tp") + s(year, River_section, bs = 'fs') +
      ...
      s(NS_MC, bs = "re"),
    data = spring,
    correlation = corARMA(form = ~ 1 | year, p = 1, q = 3),
    family = 'tw'
  )
...
res <-
  resid(TNM1_AR1$lme, type = "normalized")  #need to do on normalized resid
acf(res, lag.max = 36, main = "ACF - AR(1) errors")
pacf(res, lag.max = 36, main = "pACF- AR(1) errors")
layout(1)
...
TNBM <-  gam(TN ~ s(year, k = 10, m = 2, bs = "tp") + s(year, by = River_section, m = 1)+
      s(River_section, bs = 're') +
      s(DOY, bs = "tp")+
      s(NS_MC, bs = 're') +
      s(Longitude,Latitude,bs = "ds",m = c(1, .5),k = 10) +
...
TNM1 <-  gam(TN ~ s(year, k = 10, m = 2, bs = "tp") + s(year, by = River_section, m = 1)+
      s(River_section, bs = 're') +
      s(DOY, bs = "tp")+
      s(NS_MC, bs = 're') +
      s(Longitude,Latitude,bs = "ds",m = c(1, .5),k = 10) +
...
TNM2 <- gam(TN ~ s(year, k = 10, m = 2, bs = "tp") + s(year, by = River_section, m = 1)+
      s(River_section, bs = 're') +
      s(DOY, bs = "tp")+
      s(NS_MC, bs = 're') +
      s(Longitude,Latitude,bs = "ds",m = c(1, .5),k = 10) +
...
#Year partial effects with long + lat
draw(TNM2,select=9,residuals=TRUE,
     continuous_fill = ggplot2::scale_fill_distiller(palette = "PuOr", type = "div")) &
  theme_bw()
...
tn_gam_plot<-ggplot(tn_sm_year, aes(x = year, y = est)) +
  annotate(
    "rect",
    xmin = c(2006.176),
    xmax = c(2022.00),
...
tn_sm_year_TI <-
  smooth_estimates(TNM2, smooth = "s(year):River_sectionTI", dist = 0.05)
...
tn_sm_year_BR <-
  smooth_estimates(TNM2, smooth = "s(year):River_sectionBR", dist = 0.05)
...
tn_sm_year_CA <-
  smooth_estimates(TNM2, smooth = "s(year):River_sectionCA", dist = 0.05)
...
tn_sm_year_LSF <-
  smooth_estimates(TNM2, smooth = "s(year):River_sectionLSF", dist = 0.05)
...
tn_temp.gam_sections<-ggarrange(t_year_CA,t_year_LSF,labels=c("A","B"), nrow=2,vjust=0.2)+
  theme(plot.margin = margin(10,1,1,1))
ggsave("TN_CA.LSF_temporalGAM.svg",tn_temp.gam_sections,dpi=600,height=10,width=12 )
tn_temp.gam_sections

sm_precip <-
  smooth_estimates(TNM2, smooth = "s(Total_Precip)", dist = 0.05)
...
TN_precip_plot <- ggplot(sm_precip, aes(x = Total_Precip, y = est)) +
  ...
  geom_ribbon(alpha = 0.5,
              aes(ymin = lower, ymax = upper, fill = "confidence interval")) +
  geom_line(data = sm_precip,
            aes(x = Total_Precip, y = est, color = "GAM"),
            linewidth = 1) +
  scale_fill_manual(values = "lightblue", name = NULL) +
  scale_color_manual(values = "darkblue", name = NULL) +
...
```{r CHLA~TP relationship, warning=F, message=F}
...
```{r TP~TSS relationship, warning=F, message=F}
...
```{r pre/post dreissenid analysis, warning=F, message=F}
...
```{r dreissenid anaylsis plots,message=F, warning=F}
theme2 <-  theme(
    axis.title.x = element_blank(),
...
```{r Annual trends,message=F, warning=F}
uslr<-icefree_dreis %>% 
